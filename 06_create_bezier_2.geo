...
Macro create_bezier_outside
	...
	p1[] = Point{14};
	p2[] = {p1[0]-(hL+clen)*Cos(angle_wn), p1[1]+(hL+clen)*Sin(angle_wn), 0}; //Point{998};
	p3[] = {k*circ_rad+p5[0], 1*circ_rad+p5[1]-clen*Cos(angle_wn), 0}; //Point{999};
	p4[] = Point{16};
...
//	Printf("pline1[1] %g", pline1[1]);
	...
	pline1[] = {0., Round(10*0.8*hL)/10, 0.};	
	pline2[] = {10., Round(10*0.8*hL)/10, 0.};
...
Macro create_bezier_inside
	...
	p1[] = Point{9};
	p2[] = {p1[0]-hL*Cos(angle_wn), p1[1]+hL*Sin(angle_wn), 0}; //Point{998};
	p3[] = {k*circ_rad+p5[0], 1*circ_rad+p5[1], 0}; //Point{999};
	p4[] = Point{11};
	
	// ---- calculating intersection btw Bezier and line from Point(5) to Point(15)
	//pline1[] = Point{5};	
	pline2[] = Point{15};
	pline1[] = {pline2[0]-10., pline2[1]-10., 0};
